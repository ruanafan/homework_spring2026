...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.tensor(self.temperature).log().requires_grad_(True) # 对数值域为正负无穷，exp后值域可以稳定大于 0，确保优化和temp 不小于 0
            self.alpha_optimizer = torch.optim.Adam([self.log_alpha], lr=alpha_learning_rate)
            self.target_entropy = -self.action_dim # 原始论文里通过实验得到的一个启发式下界
            # ENDTODO 
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action_distribution = self.actor.forward(next_obs)
            next_action = next_action_distribution.rsample()
            next_qs = self.target_critic(next_obs, next_action) # target critic对Q(s_t', a_t')进行评价
            # ENDTODO
            ...
            # print("[update_critic] next_qs before q_backup_strategy", next_qs)
            next_qs = self.q_backup_strategy(next_qs)
            # print("[update_critic] next_qs after q_backup_strategy", next_qs)
        ...
        # print("[update_critic] q_values", q_values)
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        # log_prob: action_distribution.log_prob(action_sampled_from_actor)
        # target_entropy: -action_dim
        # (-log_prob) + action_dim
        # 如果熵太小，那么log_prob就会变得很小，从loss下降角度，alpha就会被迫变大，加大探索力度
        # 如果熵较大，那么后面的数字大，就会驱动alpha变小
